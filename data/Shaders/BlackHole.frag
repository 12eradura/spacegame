...
vec4 GetFireBallColor(float currTime, vec2 pos, float trailLength, float freqMult, float stretchMult, float ampMult, float power, float radius1, float radius2, vec2 velocity, float paletteCoord)
{
	...
	{		
		...
		float verticalPos = 0.99 - 0.01 * delta * delta;	
	...
	}else
	{
		...
		if(planarPos.y > 0.0)
		{
			...
			float delta =  
				GetFireDelta(currTime, planarPos, freqMult * 0.5, stretchMult, -1.0, 0.5) * (1.0 - planarPos.x)+ 
				GetFireDelta(currTime, vec2(planarPos.x - 1.0, planarPos.y), freqMult * 0.5, stretchMult, -1.0, 0.5) * planarPos.x;
...
/*	
			float delta =  
				GetFireDelta(currTime, pos, freqMult * 0.5, stretchMult, -2.5, 0.5) * (1.0 - pos.x)+ 
				GetFireDelta(currTime, vec2(pos.x - 1.0, planarPos.y), freqMult * 0.5, stretchMult, -2.5, 0.5) * pos.x;
*/			
			//delta *= min(1.0, max(0.0, 1.0 * (1.0 - pos.y)));
			//delta *= min(1.0, max(0.0, 1.0 * (0.0 + pos.y)));
